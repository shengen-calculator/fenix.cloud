...
import {PaperSize, Workbook, Worksheet} from "exceljs";
...
/**
 * Reconciliation Report XLS format
 */
export default class ReconciliationReport {
     ...
     * @param {ReconciliationParams} params
     */
    ...
    private getBucket = () =>
        admin.storage().bucket(defineString(BUCKET_ID).value());

    // Function to color a row
    private paintRow = (worksheet: excel.Worksheet,
                        columnsNumber: number, row: number) => {
        const cells =
            "ABCDEFGHIJKLMNOPRS".split("")
    ...
    private insertTotal = (worksheet: Worksheet,
                           balance: number,
                           invoiceTotal: number,
                           index: number) => {
     ...
     *
     * @return {Workbook} generated workbook
     */
    public generateWorkbook(): Workbook {
        const workbook = new excel.Workbook();
        const worksheet = workbook.addWorksheet("sheet1", {
            pageSetup: {paperSize: PaperSize.A4, orientation: "landscape"},
        });
        ...
        this.data.forEach((x) => {
            if (prevInvNumber !== x.invoiceNumber.toString() &&
                prevInvNumber !== "" && prevInvNumber !== "0") {
            ...
            if (x.invoiceNumber === 0) {
                ...
                rowValues[8] =
                    -(this.isEuroClient ? x.priceEur ?? 0 : x.priceUah ?? 0);
                this.balance -=
                    this.isEuroClient ? x.priceEur ?? 0 : x.priceUah ?? 0;
                ...
                rowValues[8] = (this.isEuroClient ?
                    x.priceEur ?? 0 : x.priceUah ?? 0) * x.quantity;
                invoiceTotal += (this.isEuroClient ?
                    x.priceEur ?? 0 : x.priceUah ?? 0) * x.quantity;
                this.balance += (this.isEuroClient ?
                    x.priceEur ?? 0 : x.priceUah ?? 0) * x.quantity;
            }
    ...
    /**
     * Upload workbook to the storage and return link to it
    ...
    public async getReportLink(workbook: Workbook): Promise<string[]> {
        ...
        await this.getBucket().upload(
            tempLocalResultFile, uploadOptions as UploadOptions);
